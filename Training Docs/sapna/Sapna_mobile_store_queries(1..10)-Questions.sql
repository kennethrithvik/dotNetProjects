...
(select COUNT(*) from orderd where customer.customerid=orderd.customerid);
-- 37. List Fav phones of Bangalorean
select productname
from product where pid in
(select pid from orderitem where orderid in
(select orderid from orderd where customerid in
...
-- 38. List all the products which were sold in the year 2013
 select productname from product where pid in
(select pid from orderitem where orderid in
(select orderid from orderd where orderdate like '2013%'));
-- 39. List all the Nokia phones orderd by 'Ravi'

select productname
 from product,company,customer,orderd,orderitem
 where product.companyid=company.companyid and 
 customer.customerid=orderd.customerid   and 
 orderd.orderid=orderitem.orderid and
 orderitem.pid=product.pid and
 company.companyname like 'nokia'
 and customer.customername like 'ravi';
...
select productname,companyname from product p,company c 
where p.companyid=c.companyid;

-- 41. List companyid, companyname, pdoructname, product price of all products
select companyid,companyname,productname,price
from company,product 
where company.companyid=product.companyid;

-- 42. List customer name, stname and city of all the customer.
select customername,stname,city
...
select customername,city from customer,customeraddress where 
customer.customerid=customeraddress.customerid and 
1>
(select COUNT(*) from orderd where customer.customerid=orderd.customerid);

-- 44. List Customer id, customer name, orderdate, of all the orders
select orderd.customerid,customername,orderdate from orderd,customer
where orderd.customerid=customer.customerid;

-- 45. List Customer id, customer name, orderdate, company and 
-- product name with qty, price of all the orders
select customer.customerid,customername,orderdate,company.companyname,productname
from customer,orderd,product,orderitem,company
where customer.customerid=orderd.customerid and
orderd.orderid=orderitem.orderid and
company.companyid=product.companyid and
orderitem.pid=product.pid;
